...
function getMoney(el) {
    ...
    return parseFloat(money, 10) * 100;
}

function preparePercentage(scope) {
    // Dá a opçao de aplicar só dentro de um escopo
    scope = scope ? scope : document;

    $(".decimal", scope).each(function () {
        $(this).change(function () {
            var val = $(this).val();
            if (val.indexOf("%") > -1) {
                var basePercStr = $(this).attr('asg-perc-base-eval');

                if (basePercStr) {
                    var base = eval(basePercStr);
                    var perc = asgParseFloat(val);

                    var result = base * perc / 100;
                    $(this).val(formatDecimal(result));
                    $(this).trigger("change");
                }
            }
        });
...
function prepareInput(){
    $("[asg-totalize-target]").change(function () {
        totalize();
    });
}
